---
title: "Theme: Task model"
tags: [pkms-design, research, adhd, theme, tasks]
created: 2026-06-10
modified: 2026-06-11
status: reacted
---

# 31 — Theme: Task model

**Why this matters for the build:** task initiation is your #1 struggle, and the task surface is where most ADHD systems die. Mark each: ✅ / ❌ / ❓.

*Reactions transferred verbatim from `exports/phase2-reading-bundle.md`, 2026-06-11. Items #3, #5, #6 got "don't fully understand" — clarifications were added inline, and on 2026-06-12 all three were re-reacted to ✅.*

1. **Notes and tasks get opposite decay policies.** Old ideas are gold; old undone tasks are poison. Knowledge persists forever; tasks expire with amnesty unless re-committed ([[11-hn]] F5, F6).
   **React:** ❌ - Old ideas can be stashed indefinitely and pop up when I search. I kind of like the concept of lacing in old ideas with my scrolling habits in content hoarder. I think that could be really powerful. For the most part undone tasks are a poison of sorts, but that means that they need to be reshaped to be more approachable. Not doing the tasks and letting it decay just builds guilt.
2. **One next action; the backlog is invisible by default.** "You never see the whole list, only the next thing your body does." Never enumerate all steps — it makes projects feel infinite ([[14-github]] F6, [[13-youtube]] F7).
   **React:** ❓ — I dont mind seeing the backlog too much, just dont make it obvious. I think the principle matters most when I have tasks that need to get done. This is where this could be very helpful.
3. **⏱ size / ▶ first action / ✓ done-when on every task** — your own most-used convention, applied universally under real pressure ([[21-job-search-distill]] JS1). Plus: store the ▶ first step AT capture time; items without one get surfaced as "needs a first step", not "overdue" ([[14-github]] F7, [[11-hn]] L2-06).
   **React:** ❓ - I dont fully understand. -> Update: ✅ AGREE *(2026-06-12, after clarification)*
   > **Clarification (Claude, 2026-06-11):** Every task carries three tiny fields: ⏱ how big it is ("10 min"), ▶ the literal first physical action ("open cli.py and find the search function"), and ✓ what finished looks like ("tests pass"). This is already how your job-search-2026 TODO.md works — this principle just says: apply it to *every* task, everywhere. The new part is the second sentence: the system tries to attach the ▶ first step at the moment you capture the task. If a task never got one, it later shows up as "needs a first step" (a 30-second decision) instead of "overdue" (guilt). The claim from the research: a task without a concrete first action is unstartable for an ADHD brain, so the system should treat "no first step" as the actual problem to surface — not lateness.
4. **Done/Stuck/Not-now is the whole interaction grammar.** Stuck = auto-subdivide; Not-now = guilt-free deferral to the back ([[19-seed-links]] SD12).
   **React:** ✅ - Yes
5. **Paused is first-class, with a written reactivation condition** — your APPLY-ENGINE-PAUSED pattern, generalized ([[21-job-search-distill]] JS9, [[13-youtube]] F12).
   **React:** ❓ — I dont fully understand. -> Update: ✅ Agreed *(2026-06-12, after clarification)*
   > **Clarification (Claude, 2026-06-11):** "Paused" means a task or project can be put to sleep *with a written wake-up condition* — "resume when the API rework ships", "revisit after interviews wrap". While paused it vanishes from active views completely: no aging, no staleness marks, no guilt. The system re-offers it only when the condition comes true or you go looking. You invented this yourself: in job-search-2026 you marked the apply engine `PAUSED` with a note on when to restart it, instead of deleting it or letting it rot on the active list. This principle just makes that an official state with first-class support, instead of a hand-written hack.
6. **Decision gates are modeled separately from tasks and surfaced first** ([[21-job-search-distill]] JS2); tiny unblocks get batched into one sitting (JS13).
   **React:** ❓ -> Update: ✅ Agreed *(2026-06-12, after clarification)*
   > **Clarification (Claude, 2026-06-11):** A decision gate is an unmade decision that other work is waiting on ("PWA-only or sync?", "which folder scheme?"). The principle: store these as their own item type — not as tasks — and show them *first*, because an open decision silently blocks everything downstream while looking like just another to-do. Your job-search TODO already does this with explicit gate items at the top. Second half: when several <5-minute unblocking items pile up, batch them into one sitting — one activation cost, several closures.
7. **A done-log (including retroactive entries) is a first-class affordance** — logging finished trivia is a proven re-initiation trick ([[11-hn]] F7, [[12-reddit]] F14, JS5).
   **React:** ✅ - Yes. Being able to mark tasks as done in my bujo felt great. Even retroactively adding tasks and marking it as done kept me going.
8. **No fake urgency primitives.** Self-imposed deadlines and pomodoro bells get rejected by the brain that made them; what works is real consequence-proximity and interest-routing ([[13-youtube]] F3, [[17-hoarder-mining]] CH7). *(Tension worth your reaction — see [[10-synthesis]] contested list.)*
   **React:** ✅ - Hard agree.

Full evidence: [[10-synthesis]] → RQ5.
