/* ============================================================================
   PKMS "daily edition" — app logic
   Vanilla JS, no framework, no build step, no external fetch.
   Fake data is inlined below using the EXACT /api/today field names confirmed
   against today.py:172-184, plus the (currently unwired) recognition_cards()
   shape from today.py:120-169, and proposed read-only payloads (see
   DATA-CONTRACT.md).
   ============================================================================ */
...
(() => {
  ...
  const TODAY = {
    ...
    next_actions: [
      ...
      // Visible action #5 of MAX_NOTES_SHOWN=8 (today.py:15). The 4 overflow
      // items past this list live in MORE_ACTIONS (mirrors /api/next-actions).
      {
    ...
    more_notes: 4, // overflow count; the actual overflow items live in MORE_ACTIONS
  };

  /// GET /api/next-actions overflow — the `more_notes` items past MAX_NOTES_SHOWN.
  /// Mirrors the proposed /api/next-actions endpoint (see DATA-CONTRACT.md §5).
  /// The real /api/today returns only MAX_NOTES_SHOWN items + a count; the
  /// overflow is fetched separately when the accordion opens.
  const MORE_ACTIONS = [
    {
  ...
  /// Round-robin of reading + resurface, capped at k=3.
  ...
  /// Derives from vault/resources/reading/*.md where frontmatter reading=="queued".
  const READING_QUEUE = [
    {
      title: "The Cost of Interrupted Work: More Faster and Worse",
      minutes: 12,
  ...
  /// Forward-only wins; goal optional and OFF by default (honors §8 settings ban).
  const PEBBLES = {
    date: "2026-06-22",
    count: 2,
  ...
  /// Recognition-first picker candidates. Derives from index mtime.
  const RECENT_NOTES = [
  ...
  const state = {
    route: "today",
    density: "calm",
    doneNotes: new Set(),       // notes marked done this session
    moreExpanded: false,        // accordion for "more one click away"
    resurfaceDismissed: false,  // not-now OR let-go
    captureSaved: false,
  };
  ...
  /// The lede renders the breadcrumb as prose. Gap logic lives here:
  /// if no daily note yesterday, soften the lede — shame-free.
  function ledeText() {
  ...
  /* ---------------------------------------------------------------------------
     4. RENDER — Today (the centerpiece)
     --------------------------------------------------------------------------- */
  function renderToday() {
    // Masthead date
    $("#masthead-date").textContent = fmtDate(TODAY.date);

    // ── Lede (re-entry breadcrumb-as-prose) ──
    const lede = ledeText();
    ...
    // ── "To fold in" — inbox as forward-only progress, never a count ──
    // We show inbox_new ONLY as "N new to fold in" + a progress chip.
    // The numerator is done_today (work already folded in this session is wins,
    // not debt). See HONESTY.md for the interpretation.
    const inbox = TODAY.inbox_new;
    const done = TODAY.done_today + state.doneNotes.size;
    const foldTotal = inbox + TODAY.done_today; // the finite batch
    const foldEl = $("#fold");
    if (inbox > 0) {
      foldEl.innerHTML =
    ...
    // ── Resurface (single ambient prompt) ──
    renderResurface();

    // ── Next actions ──
    renderActions($("#actions"), true);

    // ── More notes accordion ──
    renderMoreNotes();
  ...
  function renderRecognitionCards() {
    ...
    if (!RECOGNITION_CARDS.length) { el.innerHTML = ""; return; }
    ...
    el.innerHTML = RECOGNITION_CARDS.map((c) => {
      ...
      const cost = (c.kind === "reading" && c.minutes)
        ? `<span class="card__cost"><span class="card__cost-glyph">⏱</span> ~${c.minutes} min</span>`
        : "";
      return `
  ...
  function renderActions(listEl, leadExcluded) {
    const items = TODAY.next_actions.filter((a, i) =>
      !(leadExcluded && i === 0) // the lead is shown separately; don't duplicate
    );
  ...
  function actionHtml(a) {
    ...
    const doneWhen = a.done_when || a.text; // ✓ done-when (proposed field; falls back to text)
    return `
  ...
  function renderMoreNotes() {
    ...
    // The overflow items come from MORE_ACTIONS (mirrors the proposed
    // /api/next-actions endpoint) — NOT from slicing TODAY.next_actions,
    // which is capped at MAX_NOTES_SHOWN by today.py:183.
  ...
  function checkCleared() {
    ...
    if (allDone) {
      ...
      // Re-render pebbles inside the cleared celebration
  ...
  function router() {
    ...
    if (route === "capture") {
      // Capture is its own ramp — focus the box, never load the app.
  ...
  function toggleDone(note) {
    if (state.doneNotes.has(note)) {
      ...
      // The micro-celebration: a pebble appears. Quiet.
      // Look up the title across both visible and overflow action lists.
  ...
  function saveCapture() {
    ...
    // Fake filename mirroring the real /capture response: "saved ✓ <filename>"
    ...
    // Swap to confirmation, then reset. Never navigate.
    const wrap = $(".capture");
    ...
    // The capture value is the security feeling (§1) — confirm instantly, system owns later.
  ...
  function dismissResurface(kind) {
    ...
    if (kind === "let-go") {
      // Forever-exit, but guilt-free and reversible (5s undo window).
      // HONESTY: this 5s undo is a small extra confirmation — flagged.
      ...
      // not-now: silent, no-renag. No toast — silence is the point.
    }
  }
  ...
  function wire() {
    ...
    // Lead action (top-level) → marks the first action done
    $("#lead-action").addEventListener("click", () => {
      const lead = TODAY.next_actions[0];
      if (lead) toggleDone(lead.note);
    });

    // Delegated clicks (actions, resurface, more-notes, go-links)
    document.addEventListener("click", (e) => {
      ...
      if (t.dataset.doneToggle) {
        ...
        // Recognition-first picker — point at the one you mean.
        showToast(`→ ${t.dataset.path} (mockup: would open the note)`, { duration: 2500 });
      }
    });

    // Capture: Cmd/Ctrl+Enter saves, Esc clears
    ...
    // Capture blur-status update
    document.addEventListener("input", (e) => {
      if (e.target.id === "capture-field") {
        const status = $("#capture-status");
        if (status) status.textContent = e.target.value.trim() ? "unsaved" : "unsaved";
      }
    });
  }
